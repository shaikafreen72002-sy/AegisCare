...
interface DayPlan {
  ...
  dosesTaken: number;
  totalDoses: number;
  isCaregiverCheckpoint: boolean; // Day 3, 6, 9...
  isDoctorCheckpoint: boolean;    // Day 5, 10...
  isToday: boolean;
  doses: {
    slot: string;
    time: string;
...
export const MonthlyCalendarPlan: React.FC = () => {
  ...
  const days: DayPlan[] = Array.from({ length: 30 }, (_, idx) => {
    ...
    if (dayNum < currentDayNum) {
      // Past days completed in demo/history
      status = 'COMPLETED';
      dosesTaken = 3;
    } else if (isToday) {
  ...
  return (
    <div className="bg-white border border-[#EFEAE1] rounded-[20px] p-5 sm:p-6 shadow-[0_4px_20px_rgba(45,37,69,0.04)] space-y-5 animate-fade-in">
      ...
      {/* Automated Escalation Algorithm Banner */}
        ...
        {days.map((day) => {
          ...
          return (
            ...
            >
              ...
              {/* Safeguard Flags on Specific Days */}
